return function(center)
    ...
    local function get_two_pair_level()
        if rawget(_G,'G') and G.GAME and G.GAME.hands and G.GAME.hands['Two Pair'] then
            return G.GAME.hands['Two Pair'].level or 1
        end
        return 1
    end

    local function compute_level_bonus()
        local level = get_two_pair_level()
    ...
    center.loc_txt.name = 'Clever Joker'
    center.loc_txt.text = {
        '{C:chips}+80{} Chips if played',
        'hand contains a {C:attention}Two Pair{}',
        '{C:chips}+8{} chips for each level',
        'of the {C:attention}Two Pair{} poker hand',
        '{C:inactive}(Currently {C:chips}+#1#{} {C:inactive}Chips)'
    }

    if rawget(_G,'G') and G.localization and G.localization.descriptions
        and G.localization.descriptions.Joker and G.localization.descriptions.Joker.j_clever then
        G.localization.descriptions.Joker.j_clever.text = center.loc_txt.text
    end
    ...
    center.calculate = function(self, card, context)
        ...
        if context and context.joker_main then
            ...
            if context.poker_hands and context.poker_hands['Two Pair'] and next(context.poker_hands['Two Pair']) then
                has_two_pair = true
            end

            if has_two_pair then
                local base_chips = 80
    ...
    center.loc_vars = function(self, info_queue, card)
        local two_pair_level = 1
        if rawget(_G,'G') and G.GAME and G.GAME.hands and G.GAME.hands['Two Pair'] then
            two_pair_level = G.GAME.hands['Two Pair'].level or 1
        end
        
        local level_bonus = 8 * math.max(0, two_pair_level - 1)
    ...
    if not rawget(_G,'VP_CLEVER_PATCHED') and rawget(_G,'Card') and Card.update then
        local old_card_update = Card.update
        function Card:update(dt)
            old_card_update(self, dt)
            if self.config and self.config.center and self.config.center.key == 'j_clever' and self.ability then
                local level_bonus = compute_level_bonus()
                self.ability.extra = level_bonus
                self.ability.t_chips = 80 + level_bonus
            end
        end
        _G.VP_CLEVER_PATCHED = true
    end

    if not rawget(_G,'VP_CLEVER_CALC_PATCHED') and rawget(_G,'Card') and Card.calculate_joker then
        ...
        _G.VP_CLEVER_CALC_PATCHED = true
    end
end