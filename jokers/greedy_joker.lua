return function(center)
    local function count_diamonds_played_before(current_card, context)
        ...
        for i = 1, #context.scoring_hand do
            ...
            if card:is_suit("Diamonds") then diamonds_count = diamonds_count + 1 end
        end
        return diamonds_count
    end

    center.loc_txt = center.loc_txt or {}
    center.loc_txt.name = 'Greedy Joker'
    center.loc_txt.text = {
        'Played cards with {C:diamonds}Diamond{} suit',
        'give {X:mult,C:white}X1.05{} Mult when scored',
        'Each additional played {C:diamonds}Diamond{} card',
    ...
    center.calculate = function(self, card, context)
        if context and context.individual and context.cardarea == G.play and not context.other_card.debuff then
            if context.other_card:is_suit("Diamonds") then
                local diamonds_before = count_diamonds_played_before(context.other_card, context)
                local multiplier = 1.05 + (diamonds_before * 0.05)
                return {
                    x_mult = multiplier,
                    card = card
                }
    ...
    if rawget(_G,'G') and G.localization and G.localization.descriptions
        and G.localization.descriptions.Joker and G.localization.descriptions.Joker.j_greedy_joker then
        G.localization.descriptions.Joker.j_greedy_joker.text = center.loc_txt.text
    end
end