return function(center)
    ...
    if rawget(_G,'G') and G.localization and G.localization.descriptions
        and G.localization.descriptions.Joker and G.localization.descriptions.Joker.j_mad then
        G.localization.descriptions.Joker.j_mad.text = center.loc_txt.text
    end
    ...
    center.loc_vars = function(self, info_queue, card)
        ...
        local level_bonus = 2 * math.max(0, two_pair_level - 1)
    ...
    if not rawget(_G,'VP_MAD_PATCHED') and rawget(_G,'Card') and Card.update then
        local old_card_update = Card.update
        function Card:update(dt)
            old_card_update(self, dt)
            if self.config and self.config.center and self.config.center.key == 'j_mad' and self.ability then
                local level_bonus = compute_level_bonus()
                self.ability.extra = level_bonus
                self.ability.t_mult = 10 + level_bonus
            end
        end
        _G.VP_MAD_PATCHED = true
    end

    if not rawget(_G,'VP_MAD_CALC_PATCHED') and rawget(_G,'Card') and Card.calculate_joker then
