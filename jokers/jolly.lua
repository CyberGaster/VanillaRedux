return function(center)
    ...
    center.loc_txt.name = 'Jolly Joker'
    center.loc_txt.text = {
        '{C:mult}+8{} Mult if played',
        'hand contains a {C:attention}Pair{}',
        '{C:mult}+1{} mult for each level',
        'of the {C:attention}Pair{} poker hand',
        '{C:inactive}(Currently {C:mult}+#1#{} {C:inactive}Mult)'
    }

    if rawget(_G,'G') and G.localization and G.localization.descriptions
        and G.localization.descriptions.Joker and G.localization.descriptions.Joker.j_jolly then
        G.localization.descriptions.Joker.j_jolly.text = center.loc_txt.text
    end
    ...
    center.calculate = function(self, card, context)
        ...
        if card.ability.t_mult == nil then
            card.ability.t_mult = 8 + compute_level_bonus()
        end

        card.ability.extra = compute_level_bonus()
        card.ability.t_mult = 8 + card.ability.extra
        ...
        if context and context.joker_main then
            ...
            if has_pair then
                ...
                card.ability.t_mult = total_mult
    ...
    center.loc_vars = function(self, info_queue, card)
        ...
        local level_bonus = math.max(0, pair_level - 1)
        local total_mult = 8 + level_bonus
    ...
    center.update = function(self, card, dt)
        ...
        if card and card.ability then
            ...
            if card.ability.extra ~= new_level_bonus then
                ...
                card.ability.t_mult = 8 + new_level_bonus
                if card.set_sprites and card.config and card.config.center then
                    card:set_sprites(card.config.center)
                end
            end
            card.ability.t_mult = 8 + (card.ability.extra or 0)
        end
    end

    if not rawget(_G,'VP_JOLLY_PATCHED') and rawget(_G,'Card') and Card.update then
        local old_card_update = Card.update
        function Card:update(dt)
            old_card_update(self, dt)
            if self.config and self.config.center and self.config.center.key == 'j_jolly' and self.ability then
                local level_bonus = compute_level_bonus()
                self.ability.extra = level_bonus
                self.ability.t_mult = 8 + level_bonus
            end
        end
        _G.VP_JOLLY_PATCHED = true
    end

    if not rawget(_G,'VP_JOLLY_CALC_PATCHED') and rawget(_G,'Card') and Card.calculate_joker then
